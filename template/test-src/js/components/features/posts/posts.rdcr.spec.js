...
describe('post reducer', function () {
	...
	it('should add new item to the state', function () {
		...
		let expectedResult = { ...initialState, data: [...initialState.data, inputItem] };
		let result = posts(state, { type: `${ACTIONS.CREATE_POST}_FULFILLED`, payload: { data: inputItem } });
	...
	it('should update item within the state', function () {
		...
		let expectedResult = { ...initialState, data: [initialState.data[0], updateItem, initialState.data[2]] };
		let result = posts(state, { type: `${ACTIONS.UPDATE_POST}_FULFILLED`, payload: { data: updateItem } });
		expect(result).toEqual(expectedResult);
	});

	it('should delete item within the state', function () {
		let url = 'http://example.com/posts/1';
		let expectedResult = { ...initialState, data: [initialState.data[1], initialState.data[2]] };
		let result = posts(state, { type: `${ACTIONS.DELETE_POST}_FULFILLED`, payload: { request: { responseURL: url } } });
		expect(result).toEqual(expectedResult);
	});

	it('should get all posts', function () {
		let expectedResult = { ...initialState };
		let result = posts(state, { type: ACTIONS.GET_POSTS, payload: { data: [...initialState.data] } });
		expect(result).toEqual(expectedResult);
	});
